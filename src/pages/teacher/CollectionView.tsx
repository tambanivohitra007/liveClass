...
export default function CollectionView() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 text-gray-900 dark:text-white">
      ...
      {/* Header */}
      <div className="bg-white dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/10 shadow-sm overflow-hidden mb-8 animate-fade-in">
        ...
        {editing ? (
          ...
          <div className="p-6 flex items-start justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{coll.name}</h1>
              {coll.description && <p className="text-gray-500 dark:text-white/50">{coll.description}</p>}
              <p className="text-sm text-gray-400 dark:text-white/40 mt-2">{quizzes.length} quiz{quizzes.length !== 1 ? 'zes' : ''}</p>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => setEditing(true)}
                className="p-2.5 rounded-xl border border-gray-200 dark:border-white/10 text-gray-500 dark:text-white/60 hover:text-brand hover:border-brand/20 transition-colors"
                title="Edit"
              >
      ...
      {quizzes.length === 0 ? (
        <div className="text-center py-16">
          <EmptyCollection />
          <p className="text-gray-400 dark:text-white/40 mt-4">No quizzes in this collection yet.</p>
          <p className="text-sm text-gray-400 dark:text-white/40 mt-1">Assign quizzes from the Dashboard or Quiz Editor.</p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 stagger-children">
          {quizzes.map((quiz) => (
            <div
              key={quiz.id}
              className="bg-white dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/10 shadow-sm hover:shadow-md hover:border-brand/10 transition-all group animate-fade-in"
            >
              <div className="p-6">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="font-semibold text-gray-900 dark:text-white group-hover:text-brand transition-colors line-clamp-1">
                    {quiz.title || 'Untitled Quiz'}
                  </h3>
                  <button
                    onClick={() => handleRemoveQuiz(quiz.id)}
                    className="p-1 rounded-lg text-gray-300 dark:text-white/30 hover:text-danger hover:bg-danger/5 transition-colors shrink-0 ml-2"
                    title="Remove from collection"
                  >
                    <XIcon className="w-4 h-4" />
                  </button>
                </div>
                <p className="text-sm text-gray-400 dark:text-white/40 line-clamp-2 mb-3">
              ...
              <div className="px-6 py-3 border-t border-gray-50 dark:border-white/10 flex gap-2">
                <button
                  onClick={() => navigate(`/quiz/${quiz.id}`)}
                  className="flex-1 py-2 text-sm font-medium text-gray-600 dark:text-white/70 hover:text-brand hover:bg-brand/5 rounded-lg transition-colors"
                >
                  Edit
                </button>
                <button
                  onClick={() => navigate(`/quiz/${quiz.id}/preview`)}
                  className="px-3 py-2 text-sm text-gray-400 dark:text-white/40 hover:text-accent-dark hover:bg-accent/5 rounded-lg transition-colors"
                  title="Preview"
                >
