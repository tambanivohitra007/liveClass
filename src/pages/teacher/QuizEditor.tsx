...
export default function QuizEditor() {
  ...
  const [mobilePanel, setMobilePanel] = useState<'list' | 'editor' | 'settings'>('editor');
  ...
  const selectQuestionMobile = (i: number) => {
    setActiveIndex(i);
    setMobilePanel('editor');
  };

  const addQuestionMobile = () => {
    addQuestion();
    setMobilePanel('editor');
  };
  ...
  const renderSettings = (opts?: { showPreview?: boolean }) => (
    <div className="p-4 space-y-4">
      ...
      <div className="space-y-3">
        ...
        {opts?.showPreview && !isNew && (
          <button
            onClick={() => navigateGuard(`/quiz/${quizId}/preview`)}
            className="w-full px-4 py-2.5 border border-gray-200 dark:border-white/10 text-gray-600 dark:text-white/70 font-medium rounded-xl hover:bg-gray-50 dark:hover:bg-white/10 transition-colors flex items-center justify-center gap-2 text-sm"
          >
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-gray-50 dark:bg-surface-dark text-gray-900 dark:text-white">
      ...
      <header className="h-14 flex items-center justify-between px-4 bg-white dark:bg-white/5 border-b border-gray-200 dark:border-white/10 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            className="text-lg font-bold bg-transparent border-none outline-none text-gray-900 dark:text-white placeholder:text-gray-300 dark:placeholder:text-white/30 w-32 md:w-64 focus:ring-0"
          />
        </div>
        <div className="flex items-center gap-2">
          ...
          {!isNew && (
            <button
              onClick={() => navigateGuard(`/quiz/${quizId}/preview`)}
              className="hidden md:flex px-4 py-2 border border-gray-200 dark:border-white/10 text-gray-600 dark:text-white/70 font-medium rounded-xl hover:bg-gray-50 dark:hover:bg-white/10 transition-colors items-center gap-2 text-sm"
            >
              <Eye className="w-4 h-4" />
              Preview
            </button>
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="px-4 md:px-5 py-2 bg-brand text-white font-semibold rounded-xl hover:bg-brand-dark transition-colors disabled:opacity-50 text-sm"
          >
            {saving ? '...' : 'Save'}
          </button>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden pb-14 md:pb-0">
        {/* ── Left Sidebar – Question List (desktop) ── */}
        <aside className="w-64 bg-white dark:bg-white/5 border-r border-gray-200 dark:border-white/10 hidden md:flex flex-col shrink-0 overflow-hidden">
          {renderQuestionList()}
        </aside>

        {/* ── Mobile: Question List Panel ── */}
        {mobilePanel === 'list' && (
          <div className="flex-1 overflow-hidden flex flex-col bg-white dark:bg-white/5 md:hidden">
            {renderQuestionList({ onSelect: selectQuestionMobile, onAdd: addQuestionMobile })}
          </div>
        )}

        {/* ── Center – Question Editor ── */}
        <main className={`flex-1 overflow-y-auto ${mobilePanel !== 'editor' ? 'hidden md:block' : ''}`}>
          {activeQ ? (
            <div className="max-w-2xl mx-auto px-4 md:px-6 py-6 space-y-4 pb-6">
              {/* Mobile question indicator */}
              <p className="text-xs text-gray-400 dark:text-white/40 text-center md:hidden">
        ...
        {/* ── Right Sidebar – Quiz Settings (desktop) ── */}
        <aside className="w-64 bg-white dark:bg-white/5 border-l border-gray-200 dark:border-white/10 hidden md:flex flex-col shrink-0 overflow-y-auto">
          {renderSettings()}
        </aside>

        {/* ── Mobile: Settings Panel ── */}
        {mobilePanel === 'settings' && (
          <div className="flex-1 overflow-y-auto bg-white dark:bg-white/5 md:hidden">
            {renderSettings({ showPreview: true })}
          </div>
        )}
      </div>
      ...
      <div
        className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-white dark:bg-surface-dark border-t border-gray-200 dark:border-white/10"
        ...
        <div className="h-14 flex items-center justify-around">
          <button
            ...
            className={`flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-lg transition-colors ${
              mobilePanel === 'list' ? 'text-brand' : 'text-gray-400 dark:text-white/40'
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-lg transition-colors ${
              mobilePanel === 'editor' ? 'text-brand' : 'text-gray-400 dark:text-white/40'
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-lg transition-colors ${
              mobilePanel === 'settings' ? 'text-brand' : 'text-gray-400 dark:text-white/40'
            }`}
          >
