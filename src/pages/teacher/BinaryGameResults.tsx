...
import {
  Binary, Trophy, Users, Target, CheckCircle2, XCircle, Clock, Zap, Hash,
} from 'lucide-react';
import type { BinaryGame, BinaryGamePlayer, BinaryGameAnswer, BinaryConversionType } from '../../types/models';
...
export default function BinaryGameResults() {
  const { binaryGameId } = useParams<{ binaryGameId: string }>();
  const navigate = useNavigate();

  const [game, setGame] = useState<BinaryGame | null>(null);
  const [players, setPlayers] = useState<BinaryGamePlayer[]>([]);
  const [answers, setAnswers] = useState<BinaryGameAnswer[]>([]);
  const [loading, setLoading] = useState(true);
  const [tab, setTab] = useState<'leaderboard' | 'rounds'>('leaderboard');

  useEffect(() => {
    if (!binaryGameId) return;

    const load = async () => {
      try {
        const [gameSnap, playersSnap, answersSnap] = await Promise.all([
          getDoc(doc(db, 'binary_games', binaryGameId)),
          getDocs(collection(db, `binary_games/${binaryGameId}/players`)),
          getDocs(collection(db, `binary_games/${binaryGameId}/answers`)),
        ]);
        ...
        setGame({ id: gameSnap.id, ...gameSnap.data() } as BinaryGame);
        setPlayers(playersSnap.docs.map((d) => ({ id: d.id, ...d.data() })) as BinaryGamePlayer[]);
        setAnswers(answersSnap.docs.map((d) => d.data()) as BinaryGameAnswer[]);
      } catch {
        navigate('/history');
      } finally {
        setLoading(false);
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-gray-500 dark:text-white/50 text-sm">
              {game.difficulty} &middot; {game.roundCount} rounds &middot; PIN {game.pinCode}
              {game.endedAt && (
