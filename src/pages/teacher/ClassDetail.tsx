...
import {
  Copy, RefreshCw, Trash2, UserPlus, Users, Shield,
...
export default function ClassDetail() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 text-gray-900 dark:text-white">
      ...
      {/* Header Card */}
      <div className="bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 shadow-sm overflow-hidden mb-6 animate-fade-in">
        ...
        {editing ? (
          <div className="p-6 space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-white/20 bg-white dark:bg-white/5 focus:ring-2 focus:ring-brand/30 focus:border-brand outline-none text-gray-900 dark:text-white resize-none"
              />
            </div>
            <div>
              ...
              <div className="flex gap-2">
                {COLLECTION_COLORS.map((c) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full ${c.bg} transition-all ${editColor === c.key ? 'ring-2 ring-offset-2 ring-gray-400 dark:ring-white/50 dark:ring-offset-surface-dark scale-110' : 'hover:scale-105'}`}
                    title={c.label}
                  />
                ))}
              </div>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-5 py-2.5 border border-gray-200 dark:border-white/10 text-gray-600 dark:text-white/70 font-medium rounded-xl hover:bg-gray-50 dark:hover:bg-white/10 transition-colors"
              >
                Cancel
              </button>
            </div>
          ...
          <div className="p-6 flex items-start justify-between">
            <div>
              ...
              {classroom.description && <p className="text-gray-500 dark:text-white/50">{classroom.description}</p>}
              <div className="flex items-center gap-4 text-sm text-gray-400 dark:text-white/40 mt-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-2 rounded-xl border border-gray-200 dark:border-white/10 text-sm text-gray-500 dark:text-white/60 hover:text-brand hover:border-brand/20 transition-colors"
              >
                Edit
              </button>
              {isOwner && (
                <button
                  ...
                  className="p-2.5 rounded-xl border border-gray-200 dark:border-white/10 text-gray-500 dark:text-white/60 hover:text-danger hover:border-danger/20 transition-colors"
                  title="Delete class"
                >
      ...
      {/* Join Code Card */}
      <div className="bg-white dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/10 shadow-sm p-6 mb-6 animate-fade-in">
        <h2 className="text-sm font-semibold text-gray-500 dark:text-white/50 uppercase tracking-wider mb-4">Join Code</h2>
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <div className="flex-1 flex items-center gap-3">
            <span className="text-4xl font-black tracking-[0.2em] text-gray-900 dark:text-white font-mono">{classroom.joinCode}</span>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 text-sm font-medium text-gray-600 dark:text-white/70 hover:bg-gray-50 dark:hover:bg-white/10 hover:border-brand/30 transition-all flex items-center gap-2"
            >
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 text-sm font-medium text-gray-600 dark:text-white/70 hover:bg-gray-50 dark:hover:bg-white/10 hover:border-brand/30 transition-all flex items-center gap-2 disabled:opacity-50"
            >
      ...
      <div className="bg-white dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/10 shadow-sm p-6 mb-6 animate-fade-in">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-gray-500 dark:text-white/50 uppercase tracking-wider flex items-center gap-2">
        ...
        <form onSubmit={handleAddCoTeacher} className="flex gap-2 mb-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none text-sm text-gray-800 dark:text-white"
            />
          </div>
          <button
            type="submit"
        ...
        {/* Owner */}
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gray-50 dark:bg-white/10 mb-2">
          <div className="w-9 h-9 bg-linear-to-br from-brand to-accent rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0">
        ...
        {coTeachers.map((member) => (
          <div key={member.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-white/10 transition-colors">
            <div className="w-9 h-9 bg-gray-200 dark:bg-white/15 rounded-full flex items-center justify-center text-gray-600 dark:text-white/80 text-xs font-bold shrink-0">
              {member.displayName?.charAt(0)?.toUpperCase() || '?'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{member.displayName}</p>
              <p className="text-xs text-gray-400 dark:text-white/40 truncate">{member.email}</p>
            </div>
            <span className="text-xs text-gray-400 dark:text-white/40">{formatDate(member.joinedAt)}</span>
            <button
              onClick={() => handleRemoveMember(member.id, member.displayName)}
              disabled={removingId === member.id}
              className="p-1.5 rounded-lg text-gray-300 dark:text-white/30 hover:text-danger hover:bg-danger/5 transition-colors disabled:opacity-50"
              title="Remove"
            >
        ...
        {coTeachers.length === 0 && (
          <p className="text-sm text-gray-400 dark:text-white/40 text-center py-3">No co-teachers yet. Add one above.</p>
        )}
      </div>

      {/* Students Section */}
      <div className="bg-white dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/10 shadow-sm p-6 animate-fade-in">
        <h2 className="text-sm font-semibold text-gray-500 dark:text-white/50 uppercase tracking-wider flex items-center gap-2 mb-4">
          <Users className="w-4 h-4" /> Students ({students.length})
        </h2>

        {students.length === 0 ? (
          <div className="text-center py-8">
            <Users className="w-10 h-10 text-gray-200 dark:text-white/20 mx-auto mb-3" />
            <p className="text-sm text-gray-400 dark:text-white/40">No students have joined yet.</p>
            <p className="text-xs text-gray-400 dark:text-white/40 mt-1">Share the join code above to invite students.</p>
          </div>
        ) : (
