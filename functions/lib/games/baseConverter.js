...
const BASE_PREFIXES = { "2": "0b", "8": "0o", "10": "", "16": "0x" };
...
function generateRounds(config, roundCount, timeLimitSec) {
    const bases = config.bases || ["2", "8", "10", "16"];
    const maxValue = config.maxValue || 255;
    const rounds = [];
...
const baseConverterModule = {
    ...
    validateConfig(config) {
        ...
        if (bases && bases.length < 2)
            throw new Error("Select at least two bases");
    },
};
exports.default = baseConverterModule;
