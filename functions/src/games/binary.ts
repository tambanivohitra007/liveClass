...
function generateBinaryRounds(
  ...
  const difficulty = (config.difficulty as string) || "8bit";
  const conversionTypes = (config.conversionTypes as string[]) || ["dec2bin", "bin2dec"];
  ...
  for (let i = 0; i < roundCount; i++) {
    ...
    switch (type) {
      case "dec2bin": prompt = decStr; answer = binStr; break;
      case "bin2dec": prompt = binStr; answer = decStr; break;
      case "dec2hex": prompt = decStr; answer = hexStr; break;
      case "hex2dec": prompt = hexStr; answer = decStr; break;
      case "hex2bin": prompt = hexStr; answer = binStr; break;
      case "bin2hex": prompt = binStr; answer = hexStr; break;
      default: prompt = decStr; answer = binStr;
    }
...
function checkBinaryAnswer(submission: string, round: GameRound): boolean {
...
const binaryModule: GameModuleServer = {
