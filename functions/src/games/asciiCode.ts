...
const COMMON_CHARS = PRINTABLE_CHARS.filter((c) =>
  (c.code >= 48 && c.code <= 57) || // 0-9
...
function generateRounds(config: Record<string, unknown>, roundCount: number, timeLimitSec: number): GameRound[] {
  ...
  const difficulty = (config.difficulty as string) || "common";
  ...
  for (let i = 0; i < roundCount; i++) {
    ...
    let type: string;
    if (direction === "char2code") type = "char2code";
    else if (direction === "code2char") type = "code2char";
    else type = Math.random() > 0.5 ? "char2code" : "code2char";
...
function checkAnswer(submission: string, round: GameRound): boolean {
  ...
  if (sub.toUpperCase() === "SPACE" && round.answer === "SPACE") return true;
  return sub === round.answer || sub.toUpperCase() === round.answer.toUpperCase();
}

const asciiCodeModule: GameModuleServer = {
