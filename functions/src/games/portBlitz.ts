...
function generateRounds(config: Record<string, unknown>, roundCount: number, timeLimitSec: number): GameRound[] {
  ...
  for (let i = 0; i < roundCount; i++) {
    ...
    let type: string;
    if (direction === "service2port") type = "service2port";
    else if (direction === "port2service") type = "port2service";
    else type = Math.random() > 0.5 ? "service2port" : "port2service";
...
function checkAnswer(submission: string, round: GameRound): boolean {
  ...
  return sub === ans || sub === (round.meta?.service as string)?.toUpperCase();
}

const portBlitzModule: GameModuleServer = {
